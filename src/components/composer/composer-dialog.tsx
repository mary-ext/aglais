...
import EarthOutlinedIcon from '~/components/icons-central/earth-outline';
import EmojiSmileOutlinedIcon from '~/components/icons-central/emoji-smile-outline';
import GifSquareOutlinedIcon from '~/components/icons-central/gif-square-outline';
import ImageOutlinedIcon from '~/components/icons-central/image-outline';
...
import ShieldCheckOutlinedIcon from '~/components/icons-central/shield-check-outline';
	...
	const addImagesOrVideo = async (blobs: Blob[]) => {
		...
		if (images.length) {
			...
			if (!next) {
				...
				next = {
					type: 'image',
					images: images.slice(0, MAX_IMAGES).map((source) => ({ source, alt: '' })),
					labels: [],
				};
			} else if (next.type === 'image') {
				onError();
				next.images = next.images.concat(
					images.slice(0, MAX_IMAGES - next.images.length).map((source) => ({ source, alt: '' })),
				);
			}
